...
import { fetchSupadataTranscript, resolveSupadataConfig } from "./supadata.js";

const LANGUAGE_PRIORITY = ["zh-Hans", "zh-CN", "zh", "zh-TW", "en"];
const MAX_STORYBOARD_IMAGES = 20;
const MAX_BROWSER_STORYBOARD_IMAGES = 10;
const MAX_BROWSER_IMAGE_BYTES = 750_000;
const MAX_BROWSER_IMAGE_TOTAL_BYTES = 2_700_000;
...
async function recognizeStoryboardImages(images, format, env) {
  ...
  const fragments = images.map(({ startSeconds, duration }) => ({ startSeconds, duration }));
  const content = [{
    type: "input_text",
...
async function fetchStoryboardTranscript(metadata, env, network) {
  ...
  return recognizeStoryboardImages(images, format, env);
}

function cleanBrowserText(value, maxLength) {
  return String(value || "").replace(/\s+/g, " ").trim().slice(0, maxLength);
}

export function normalizeBrowserYoutubeSource(source) {
  if (!source || typeof source !== "object") throw new Error("浏览器助手没有返回有效内容");
  const videoId = cleanBrowserText(source.videoId, 20);
  if (!/^[A-Za-z0-9_-]{6,20}$/.test(videoId)) throw new Error("浏览器助手返回的视频 ID 无效");
  const title = cleanBrowserText(source.title, 200);
  const author = cleanBrowserText(source.author, 120);
  const language = cleanBrowserText(source.language, 20) || "zh";
  const segments = Array.isArray(source.segments)
    ? source.segments.slice(0, 10_000).flatMap((segment) => {
      const text = cleanBrowserText(segment?.text, 2_000);
  ...
  if (segments.length && characters <= 150_000) {
    return { videoId, title, author, language, segments, extractionMethod: "browser_captions" };
  }
  if (characters > 150_000) throw new Error("浏览器提取的字幕超过 15 万字符限制");

  const storyboard = source.storyboard;
  const rows = Number(storyboard?.rows);
  const columns = Number(storyboard?.columns);
  if (!Number.isInteger(rows) || rows < 1 || rows > 10 || !Number.isInteger(columns) || columns < 1 || columns > 10) {
    throw new Error("浏览器助手返回的故事板布局无效");
  }
  if (!Array.isArray(storyboard.images) || !storyboard.images.length || storyboard.images.length > MAX_BROWSER_STORYBOARD_IMAGES) {
    throw new Error(`浏览器助手最多可提交 ${MAX_BROWSER_STORYBOARD_IMAGES} 张故事板图片`);
  }

  let totalBytes = 0;
  const images = storyboard.images.map((image) => {
    const mimeType = String(image?.mimeType || "").toLowerCase();
    if (!new Set(["image/jpeg", "image/png", "image/webp"]).has(mimeType)) {
      throw new Error("浏览器助手返回了不支持的图片格式");
    }
    const encoded = String(image?.data || "");
    if (!encoded || !/^[A-Za-z0-9+/]+={0,2}$/.test(encoded)) throw new Error("浏览器助手返回的图片数据无效");
    const buffer = Buffer.from(encoded, "base64");
    if (!buffer.length || buffer.length > MAX_BROWSER_IMAGE_BYTES) throw new Error("浏览器助手返回的单张图片过大");
    totalBytes += buffer.length;
    const startSeconds = Number(image.startSeconds);
    const duration = Number(image.duration);
    if (!Number.isFinite(startSeconds) || startSeconds < 0 || !Number.isFinite(duration) || duration <= 0) {
      throw new Error("浏览器助手返回的故事板时间信息无效");
    }
    return { mimeType, buffer, startSeconds, duration };
  });
  if (totalBytes > MAX_BROWSER_IMAGE_TOTAL_BYTES) throw new Error("浏览器助手返回的图片总大小超过限制");
  ...
  return {
    ...
    author,
    language,
    storyboard: { rows, columns, images }
  };
}

export async function fetchYoutubeTranscriptFromBrowser(source, env = process.env) {
  const normalized = normalizeBrowserYoutubeSource(source);
  if (normalized.segments) return normalized;
  const result = await recognizeStoryboardImages(normalized.storyboard.images, normalized.storyboard, env);
  return {
    videoId: normalized.videoId,
    title: normalized.title,
    author: normalized.author,
    ...result,
    extractionMethod: "browser_storyboard_ocr"
  };
}
...
function youtubeError(error, network = {}) {
  ...
  if (/sign in|confirm you.re not a bot|cookies|登录|登入|机器人|機器人|验证|驗證/i.test(detail)) {
...
async function fetchYoutubeMetadata(videoId, network) {
  ...
  try {
    ...
    if (error.youtubeCode) throw error;
    failures.push(error);
  }
...
const SUPADATA_NO_FALLBACK_CODES = new Set([
  "SUPADATA_INVALID_REQUEST",
...
export async function fetchYoutubeTranscriptAuto(videoId, env = process.env, onProgress = () => {}) {
  if (!resolveSupadataConfig(env).enabled) return fetchYoutubeTranscript(videoId, env);
  try {
    return await fetchSupadataTranscript(videoId, env, onProgress);
  } catch (error) {
    if (SUPADATA_NO_FALLBACK_CODES.has(error.youtubeCode)) throw error;
    console.warn(
      `[youtube] Supadata 读取失败（${error.youtubeCode || "unknown"}），改为尝试 YouTube 直连:`,
      error.message
    );
  }
